...
// Shared fetch helper
// ---------------------------------------------------------------------------
async function eb<T>(path: string, apiKey: string): Promise<{ ok: boolean; status: number; data: T }> {
...
const handler: Handler = async () => {
  ...
  if (!apiKey) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: "Eventbrite API key not configured" }) };
  }

  const eventParams = "?status=live&time_filter=current_future&expand=venue,category&order_by=start_asc";
  ...
  try {
    ...
    if (!userOk || !user.id) {
      ...
      return { statusCode: 502, headers, body: JSON.stringify({ error: "Could not authenticate with Eventbrite" }) };
    }

    const userId = user.id;

    // Step 2 — try to get the first organization this user belongs to
    const { ok: orgsOk, data: orgsData } = await eb<{ organizations?: { id: string }[] }>(
      `/users/${userId}/organizations/`,
      apiKey
    );

    const orgId = orgsOk ? (orgsData as { organizations?: { id: string }[] }).organizations?.[0]?.id : undefined;
    ...
    if (!eventsOk) {
      ...
      return {
        statusCode: 502,
        headers,
        body: JSON.stringify({ error: eventsData.error_description ?? "Failed to fetch events" }),
      };
    }
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: "Failed to fetch events from Eventbrite" }) };
  }
};
