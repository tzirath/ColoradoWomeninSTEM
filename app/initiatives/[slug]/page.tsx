...
const initiatives: Record<string, {
...
export function generateStaticParams() {
  return Object.keys(initiatives).map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const item = initiatives[params.slug];
  if (!item) return {};
  return {
    title: `${item.title} | CWS`,
    ...
    <div className="min-h-screen bg-background pt-24">
      <section className="py-20">
        <div className="container mx-auto px-6 max-w-3xl">
          <Link
            href="/initiatives"
            className="inline-flex items-center gap-2 text-foreground/60 hover:text-primary font-body text-sm mb-10 transition-colors"
          >
          ...
          <p className="font-body text-secondary text-sm uppercase tracking-[0.2em] mb-2 italic">
            {item.tagline}
          </p>
          ...
          <div className="space-y-6">
            {item.body.map((para, i) => (
              <p key={i} className="font-body text-foreground text-lg leading-relaxed">
                {para}
              </p>
            ))}
          </div>
          ...
          <div className="mt-14 p-8 bg-card rounded-2xl border border-border">
            ...
            <p className="font-body text-foreground/80 mb-6">
              Interested in participating in or supporting this initiative?
            </p>
            <Link
              href={item.getInvolvedHref}
